...
# Contoh penggunaan sederhana (bisa dikomentari setelah pengujian awal)
# if __name__ == '__main__':
#     # Buat file dummy untuk pengujian
#     nama_file_dummy = "dummy_test_file.txt"
#     with open(nama_file_dummy, "w") as f:
#         f.write("Ini adalah konten tes.")

#     kunci = buat_kunci_cache_file(nama_file_dummy)
#     print(f"Kunci Cache untuk {nama_file_dummy}: {kunci}")

#     if kunci:
#         data_tes = {"info_penting": "Ini adalah data yang akan di-cache.", "angka": 12345}
#         data_untuk_simpan = {'teks_dokumen': data_tes, 'timestamp_pembuatan_cache': time.time()}

#         berhasil_simpan = simpan_ke_cache(kunci, data_untuk_simpan)
#         print(f"Status penyimpanan ke cache: {berhasil_simpan}")

#         data_diambil = ambil_dari_cache(kunci)
#         if data_diambil:
#             print(f"Data berhasil diambil dari cache: {data_diambil.get('teks_dokumen')}")
#             print(f"Timestamp cache: {datetime.fromtimestamp(data_diambil.get('timestamp_pembuatan_cache')) if data_diambil.get('timestamp_pembuatan_cache') else 'N/A'}")
#         else:
#             print("Data tidak ditemukan di cache atau error.")

#         # Ubah file dummy untuk mendapatkan kunci cache baru
#         time.sleep(1) # Pastikan timestamp modifikasi berbeda
#         with open(nama_file_dummy, "a") as f:
#             f.write("\nKonten tambahan.")

#         kunci_baru = buat_kunci_cache_file(nama_file_dummy)
#         print(f"Kunci Cache Baru setelah modifikasi file: {kunci_baru}")
#         data_diambil_lagi = ambil_dari_cache(kunci_baru)
#         if data_diambil_lagi:
#             print(f"Mencoba mengambil dengan kunci baru (seharusnya tidak ada): {data_diambil_lagi}")
#         else:
#             print("Cache dengan kunci baru tidak ditemukan (sesuai harapan).")

#         # Tes pembersihan cache (buat beberapa file cache dummy)
#         simpan_ke_cache("kunci_lama_1", {"data":"lama", "timestamp_pembuatan_cache": time.time() - (35 * 24 * 60 * 60)})
#         simpan_ke_cache("kunci_lama_2", {"data":"sangat_lama", "timestamp_pembuatan_cache": time.time() - (70 * 24 * 60 * 60)})
#         bersihkan_cache_lama(batas_usia_detik=2 * 24 * 60 * 60) # Hapus yang lebih tua dari 2 hari (dummy akan dihapus)
#         # bersihkan_cache_lama() # Hapus yang lebih tua dari 30 hari (default)


#     # Hapus file dummy setelah tes
#     if os.path.exists(nama_file_dummy):
#         os.remove(nama_file_dummy)
#     if os.path.exists(NAMA_DIREKTORI_CACHE_DEFAULT):
#         # Hati-hati jika direktori cache ini digunakan oleh proses lain
#         # Untuk pengujian, kita bisa menghapusnya. Dalam produksi, mungkin tidak.
#         # import shutil
#         # shutil.rmtree(NAMA_DIREKTORI_CACHE_DEFAULT)
#         print(f"Direktori cache '{NAMA_DIREKTORI_CACHE_DEFAULT}' mungkin berisi file cache. Periksa manual jika perlu.")

#     # Perlu import datetime jika ingin menjalankan contoh di atas dengan format timestamp
#     # from datetime import datetime
# Komentar penutup yang salah telah dihapus.
